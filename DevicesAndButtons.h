...
public:
	...
	ALLEGRO_COLOR color_text_with_mouse;
	const char *text;
	...
	void put_text(ALLEGRO_FONT *f, ALLEGRO_COLOR c_text, ALLEGRO_COLOR c_text_wm, const char *t);
...
void Button::put_text(ALLEGRO_FONT *f, ALLEGRO_COLOR c_text, ALLEGRO_COLOR c_text_wm, const char * t){
	font = f;
	color_text = c_text;
	color_text_with_mouse = c_text_wm;
	text = t;
}
...
void Buttons::draw_font(mouses m) {	
	Button* t = posl;
	for (; t; t = t->last) {
		al_draw_text(t->font, (t->collide(m))?(t->color_text_with_mouse):(t->color_text), 
			t->x1 + abs((t->x1 - t->x2)/2), t->y1 + abs((t->y1 - t->y2)/2) - 12, 1, t->text);
	}
